...
import com.cooder.cooder.project.app.databinding.ItemDetailShopBinding
...
class ShopItem(private val model: DetailModel) : CoDataItem<DetailModel, CoViewHolder>() {
	...
	private fun spanTag(context: Context, name: String, level: String): CharSequence {
		...
		ssLevel.setSpan(ForegroundColorSpan(ContextCompat.getColor(context, R.color.detail_shop_tag_level_fg)), 0, ssLevel.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
		ssLevel.setSpan(BackgroundColorSpan(ContextCompat.getColor(context, R.color.detail_shop_tag_level_bg)), 0, ssLevel.length, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
